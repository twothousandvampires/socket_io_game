...
io.sockets.on('connection', (socket) => {
        ...
        if (level.users.length < 20) {
            if (data.data) {
                io.sockets.emit('updateChat', {
                    data: 'игрок ' + data.data.nik + ' присоединился'
                })
                ...
                io.sockets.emit('updateChat', {
                    data: 'неизвестный игрок присоединился'
                })
                let randomSpot = level.massSpots[Math.floor(Math.random() * level.massSpots.length)];
    ...
    setInterval(() => {
        ...
        if (!checkWinner()) {
            ...
            io.sockets.emit('callInfo');
            io.sockets.emit("draw", {
                data: level
            });
        } else {
            io.sockets.emit("loadScoreBoard", {
                data: level
            })
        }
...
function Actor(name, type, x, y, sizeX, sizeY, vector, owner) {
    this.name = name;
    this.type = type;
    this.pos = new Vector(x, y);
    this.size = new Vector(sizeX, sizeY);
    this.moveSpeed = vector;
    this.owner = owner;
}
